...
static inline std::string random_string(std::size_t length, int seed = -1) {
  ...
  std::uniform_int_distribution<size_t> distribution(size_t(0),
                                                     CHARACTERS.size() - 1);
...
to_lower_branchless(std::string &sv) {
  char *d = sv.data();
...
[[maybe_unused]] static inline const char *
to_upper_branchless(std::string &sv) {
  if (sv.empty())
    return nullptr;
  char *d = &sv[0];
...
[[maybe_unused]] static inline void to_upper_inplace(std::string_view sv) {
  if (sv.empty())
    return;
  char *d = (char *)&sv[0];
  for (auto i = 0; i < (int)sv.size(); ++i) {
...
[[maybe_unused]] static inline const char *
flip_case_branchless(std::string &sv) {
  if (sv.empty())
    return nullptr;
  char *d = &sv[0];
  for (auto i = 0; i < (int)sv.size(); ++i) {
...
[[maybe_unused]] static inline std::string_view
ltrim(const std::string_view sv,
...
[[maybe_unused]] static inline std::string_view
rtrim(const std::string_view sv,
      const std::string_view trim_what = " \t\r\v\n") {
  std::string_view s{sv};
  s.remove_suffix(
      (std::min)(s.size() - s.find_last_not_of(trim_what) - 1, s.size()));
  return s;
}
...
[[maybe_unused]] static inline std::string trim_copy(std::string s) noexcept {
  trim(s);
  return s;
}
// trim from both ends (copying)
[[maybe_unused]] static inline std::string
trim_copy(const std::string_view sv) noexcept {
  std::string s(sv);
  trim(s);
  return s;
}
...
struct string_hash_transparent_ci {
  ...
  template <typename T> [[nodiscard]] size_t operator()(T &&) const noexcept {
    assert(0);
    return 0;
  }
};
...
template <> struct ci_equal_to<void> {
  ...
  [[nodiscard]] static bool compare(std::string left,
  ...
  [[nodiscard]] bool operator()(T1 &&left, T2 &&right) const noexcept {
...
static inline bool compare_less_nocase(const std::string_view a,
                                       const std::string_view b) noexcept {
  // ReSharper disable once CppUseRangeAlgorithm
  return std::lexicographical_compare(a.begin(), a.end(), b.begin(), b.end(),
                                      compare_less_nocase_c);
}
...
static inline std::string concat_into(std::stringstream &ss, Args &&...args) {
  ((ss << args), ...);
...
#define THROW_ERROR(...)                                                       \
  utils::strings::throw_runtime_error(__FILE__, ':', __LINE__, "\r\n",         \
                                      __VA_ARGS__)
...
static inline std::vector<RESULT_TYPE>
split2(const std::string_view haystack, const std::string_view needle = " ") {
...
template <typename RESULT_TYPE>
static inline std::vector<RESULT_TYPE>
split(std::string_view str, const std::string_view delimiters = " ") {
  std::vector<RESULT_TYPE> tokens;
...
[[maybe_unused]] static inline std::string
replace_all_copy(std::string subject, const std::string &search,
...
[[maybe_unused]] static inline void escape(const std::string_view s,
                                           std::string &out) {
  out = s;
  replace_all(out, "'", "''");
  replace_all(out, "\"", "\"\"");
...
[[maybe_unused]] inline const nvp_iterator
find_if_ci(nvp_iterator first, nvp_iterator last,
...
[[maybe_unused]] static inline const nvp_iterator
contains(const name_value_pairs_t<> &v, const std::string_view find_what) {
  return find_if_ci(v.begin(), v.end(), find_what);
}

[[maybe_unused]] static inline name_value_pairs_t<> parse_args(const int argc,
                                                               char **argv) {
...
handle_max(const T ctr, const T max_ctr, const std::string &msg,
...
[[maybe_unused]] static inline std::string
file_get_name(const std::string_view filepath,
  ...
  if (const auto last_slash_pos = filepath.find_last_of(SEP);
    ...
    retval =
        retval.substr(last_slash_pos + 1, retval.size() - last_slash_pos - 1);
  }
  if (!include_extn) {
...
[[maybe_unused]] static inline std::string
file_copy(std::string_view path, std::string_view extn = ".copy") {
...
[[maybe_unused]] static inline std::string
get_temp_filename(const std::string &extn = ".txt",
...
// watch it! I throw
[[maybe_unused]] static inline std::system_error
file_move(const std::string &from, const std::string &to,
...
[[maybe_unused]] static inline std::system_error
file_open(std::fstream &f, const std::string &file_path,
          std::ios_base::openmode mode, bool throw_on_fail = false) {
  if (file_path.empty()) {
    auto err =
...
get_data_len(const std::vector<std::string_view> &d) {
  size_t ret = 0;
  for (const auto &sv : d) {
    ret += sv.size();
  }
...
[[maybe_unused]] static inline std::system_error
file_write_all(const std::vector<std::string_view> &data,
...
file_read_all(std::fstream &f, std::string &data, bool close_when_done = true) {
  data.clear();
