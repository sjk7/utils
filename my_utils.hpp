...
[[maybe_unused]] static inline std::string_view ltrim(const std::string_view sv,
...
[[maybe_unused]] static inline std::string_view rtrim(const std::string_view sv,
                                                      const std::string_view trim_what = " \t\r\v\n")
{
    std::string_view s{sv};
...
[[maybe_unused]] static inline std::string_view trim(const std::string_view sv,
                                                     const std::string_view trim_what = " \t\r\v\n")
{
...
// trim from both ends (copying)
[[maybe_unused]] static inline std::string trim_copy(const std::string_view sv) noexcept
{
    std::string s(sv);
    trim(s);
    return s;
...
template <> struct ci_equal_to<void>
{
    ...
    template <typename T1, typename T2> [[nodiscard]] bool operator()(T1 &&left, T2 &&right) const noexcept
    {
...
template <class C, class T> auto contains_nocase(const C &v, const T &x) -> decltype(end(v), true)
{
...
static inline bool compare_less_nocase(const std::string_view a, const std::string_view b) noexcept
{
...
template <typename N = std::string, typename V = std::string> struct name_value_pair
{
    N name;
    V value;
    inline friend std::ostream &operator<<(std::ostream &os, const name_value_pair &nvp)
    {
...
[[maybe_unused]] static inline nvp_iterator contains(const name_value_pairs_t<> &v, const std::string_view find_what)
{
...
[[maybe_unused]] static inline std::string file_copy(std::string_view path, std::string_view extn = ".copy")
{
...
[[maybe_unused]] static inline std::string get_temp_filename(const std::string &extn = ".txt",
...
// watch it! I throw
[[maybe_unused]] static inline std::system_error file_move(const std::string &from, const std::string &to,
                                                           bool delete_dest = true) noexcept(false)
{
    ...
    MYASSERT(r == 0, my::utils::strings::concat("file_move: cannot delete dest", errno).c_str())
    if (r)
    {
        auto e = std::system_error(errno, std::system_category(), std::string("cannot delete file ") + from);
        throw e;
    }
                                                           ...
                                                           std::ios_base::openmode mode, bool throw_on_fail = false)
{
    ...
    if (!f)
    {
        auto e = std::system_error(errno, std::system_category(), std::string("failed to open ") + file_path);
        if (throw_on_fail)
        {
            throw e;
        }
...
[[maybe_unused]] static inline size_t get_data_len(const std::vector<std::string_view> &d)
{
    size_t ret = 0;
    for (const auto &sv : d)
    {
                                                                ...
                                                                const std::string &filepath, bool throw_on_fail = false)
{
    ...
    if (!f)
    {
        auto e = std::system_error(errno, std::system_category(), std::string("failed to open ") + filepath);
        if (throw_on_fail)
        {
